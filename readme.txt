=== WP Scribd ===
Contributors: Maurizio Pelizzone
Donate link: http://maurizio.mavida.com
Tags: Scribd, pdf, document
Requires at least: 2.5
Tested up to: 2.6.1
Stable tag: 0.1

== Description ==

Description: Add Scribd docs inside a post

== Installation ==

1. Upload contents of the ZIP file to your wp-content/plugins folder.
2. Activate the "WordPress Holidays" plugin
3. Go the the WordPress Holidays Option admin menu and check is it ok...

if you want upgrade manualy:

1. Deactivate plugin
2. Delete old files
3. Upload new files

== Frequently Asked Questions ==

= To be, or not to be: that is the question =

i dunno...

== Screenshots == 

We're taking this one step further. 

http://maurizio.mavida.com/wp-scribd/

== Bugs/Support ==

Use the page at http://maurizio.mavida.com/wp-scribd for bug reports,
suggestions or feature requests.

== Changelog ==

0.1    2008/08/06 	Initial release

Thanks to all who sent bug reports and ideas for improvements.
Please send me a mail if I forgot you to mention here.

== Readme Validator ==

This readme was made using:
wordpress.org/extend/plugins/about/validator/

Totally totally cool tool!!